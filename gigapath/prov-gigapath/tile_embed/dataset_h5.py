...
class Dataset_All_Bags(Dataset):
    def __init__(self, csv_path, num_splits=1, split_no=0):
        self.df = pd.read_csv(csv_path)
        
        if num_splits > 1:
            indices = np.arange(len(self.df))
            split_indices = np.array_split(indices, num_splits)
            this_split = split_indices[split_no]
            
            self.df = self.df.iloc[this_split].reset_index(drop=True)
            print(f"Job {split_no}/{num_splits-1}: Processing {len(self.df)} slides.")
