...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
                            ...
                            use_hybrid=True, h_range=(120,175), s_min=10, v_min=120, a_min=8):#-5):
        """
        ...
        # # Remove markers/artifacts using Hybrid criteria
        if use_hybrid:
            # 1. Prepare Data
            img_lab = cv2.cvtColor(img, cv2.COLOR_RGB2LAB)
            a_chan = img_lab[:,:,1].astype(np.float32)
            h, s, v = img_hsv[:,:,0], img_hsv[:,:,1], img_hsv[:,:,2]

            # 2. Define/Scale Thresholds for OpenCV
            # If your input h_range is already in OpenCV scale (0-179), no math is needed:
            h_min_cv, h_max_cv = h_range[0], h_range[1]
            # LAB 'a' threshold (128 is neutral in OpenCV)
            a_thresh_cv = a_min + 128

            # # 3. Debug Histograms with Color Mapping
            # if True: 
            #     import matplotlib.pyplot as plt
            ...
            #     # 1. Define the boolean masks based on your thresholds
            #     mask_h = (h >= h_min_cv) & (h <= h_max_cv)
            #     mask_s = (s >= s_min)
            #     mask_v = (v >= v_min) & (v <= 248)
            #     mask_a = (a_chan >= a_thresh_cv)

            #     # 2. Foreground-only channels (Otsu mask applied)
            #     otsu_mask = img_otsu > 0
            #     h_fg = np.where(otsu_mask, h, 0)
            #     s_fg = np.where(otsu_mask, s, 0)
            #     v_fg = np.where(otsu_mask, v, 0)
            #     a_fg = np.where(otsu_mask, a_chan, 0)

            #     # 3. Apply individual thresholds to the foreground
            #     h_thresh = np.where(otsu_mask & mask_h, h, 0)
            #     s_thresh = np.where(otsu_mask & mask_s, s, 0)
            #     v_thresh = np.where(otsu_mask & mask_v, v, 0)
            #     a_thresh = np.where(otsu_mask & mask_a, a_chan, 0)

            #     if np.any(otsu_mask):
            #         # Main Figure: 1 row, 4 columns (one for each channel)
            #         fig = plt.figure(figsize=(28, 12))
            #         outer_grid = fig.add_gridspec(1, 4, wspace=0.2)

            #         channel_data = [
            #             ("Hue", h_fg, h_thresh, 'hsv', f"{h_min_cv}-{h_max_cv}"),
            #             ("Saturation", s_fg, s_thresh, 'plasma', f"> {s_min}"),
            #             ("Value", v_fg, v_thresh, 'gray', f"{v_min}-248"),
            #             ("LAB A", a_fg, a_thresh, 'RdYlGn_r', f"> {a_thresh_cv}")
            #         ]

            #         for i, (name, fg_img, th_img, colormap, label) in enumerate(channel_data):
            #             # Create a sub-grid for the "Dual Plot" (2 rows, 1 col) inside the column
            #             inner_grid = outer_grid[0, i].subgridspec(2, 1, hspace=0.1)
                        
            #             # TOP: Full Foreground
            #             ax_top = fig.add_subplot(inner_grid[0, 0])
            #             im_top = ax_top.imshow(fg_img, cmap=colormap)
            #             ax_top.set_title(f"{name}: Otsu FG", fontsize=14)
            #             ax_top.axis('off')
            #             plt.colorbar(im_top, ax=ax_top, fraction=0.046, pad=0.04)

            #             # BOTTOM: Thresholded Foreground
            #             ax_bot = fig.add_subplot(inner_grid[1, 0])
            #             im_bot = ax_bot.imshow(th_img, cmap=colormap)
            #             ax_bot.set_title(f"Proposed: {label}", fontsize=12, color='green' if np.any(th_img) else 'red')
            #             ax_bot.axis('off')
            #             plt.colorbar(im_bot, ax=ax_bot, fraction=0.046, pad=0.04)

            #         plt.suptitle("Threshold Impact Analysis: Initial Otsu vs. Channel Filtering", fontsize=20, y=0.98)
                    
            #         # --- SAVE LOGIC ---
            #         plot_save_path = '/gscratch/scrubbed/juampablo/corebt/corebt_clam_preprocessing/tiles_v6/dual_threshold_debug_0.png'
            #         dirname = os.path.dirname(plot_save_path)
            ...
            #             counter += 1
            #             plot_save_path = os.path.join(dirname, f'dual_threshold_debug_{counter}.png')

            #         plt.savefig(plot_save_path, bbox_inches='tight', dpi=150)
            #         plt.close()

            # 4. Create the refinement mask and apply
            refine_mask = (
                (h >= h_min_cv) & (h <= h_max_cv) &
                (s >= s_min) &
                (v >= v_min) & (v < 248) &
                (a_chan >= a_thresh_cv) 
            ).astype(np.uint8) * 255

            img_otsu = cv2.bitwise_and(img_otsu, refine_mask)
